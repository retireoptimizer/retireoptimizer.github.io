...
export const RELEASES: Release[] = [
  {
    version: '2.1.0',
    date: '2026-09-17',
    summary: 'Monte Carlo overhaul with a two-step Simulate and Optimize flow, robustness comparison with a clear threshold before anything is offered, Roth conversion accuracy fix eliminating phantom round-trips, optimizer policy freshness gating, New York state tax accuracy, and five targeted fixes.',
    changes: [
      { kind: 'feature', text: 'Monte Carlo page redesigned as a two-step flow: Simulate then Optimize. Step 2 is locked until you run a simulation. The optimizer searches withdrawal order and Roth conversion combinations across 32 market histories and reports the result as a compact strip: green with an Apply option when the gain clears one full percentage point (the margin of error at 5,000 trials), grey with an explanation when it does not. A result is never offered when the improvement is indistinguishable from sampling noise.' },
      { kind: 'feature', text: 'Roth conversion accuracy fix: the optimizer was recommending conversions that accomplished nothing — converting money into Roth and then immediately withdrawing it to cover spending in the same year. On some plans this affected every conversion year. Phantom conversions are now prevented before they are sized, and if skipping conversions entirely produces a higher balance, the optimizer now ships that result instead of discarding it. The Roth Converted figure on the dashboard now reflects only real repositioning.' },
      { kind: 'feature', text: 'Optimizer policy freshness: an optimizer result built against one set of inputs is now explicitly invalidated when inputs change. A fingerprint of the plan inputs is stored with the policy; if it no longer matches, a gate blocks the Dashboard, Projections, Tax Planning, and Monte Carlo pages until you re-run or discard the stale result. This replaces the prior behavior where a stale policy was silently applied to new inputs, sometimes making the plan worse.' },
      { kind: 'feature', text: 'New York state tax accuracy: three errors corrected. (1) The $20,000 per-person retirement income exclusion is now applied — a couple with $80K of IRA withdrawals was overpaying by roughly $2,600/year. (2) The NY standard deduction ($8,000 single / $16,050 MFJ) is now applied before bracket calculation. (3) NY\'s actual progressive bracket structure (4% to 9.65%) replaces the prior flat 6.5% approximation. Combined, these changes can reduce projected NY state tax by $3,000–$5,000 per year for a typical retired couple.' },
      { kind: 'feature', text: 'Pay IRA withdrawal taxes from brokerage is now available on the Inputs page, in the Optimization Goal section — no longer buried on the dashboard. This option defaults to on for all new plans, which is the better strategy for most households.' },
      { kind: 'fix', text: 'Optimizer no longer ships withdrawal splits that cannot be honored. When the optimizer recommended drawing more from a bucket than it contained, the projection engine silently corrected the shortfall rather than reporting it. The optimizer now validates splits against actual bucket balances before selecting a winner.' },
      { kind: 'fix', text: 'Optimizer preset screens (bracket-fill, Roth-first, etc.) are no longer discarded when they beat the main search incumbent. A scoring bug caused valid preset results to be ignored even when they outperformed the coordinate-descent result; all competitors now participate in the final selection.' },
      { kind: 'fix', text: 'Monte Carlo equity mix now persists as a simulation preference rather than a plan field. Changing the stock/bond split on the Monte Carlo page previously reset to 60% on reload or navigation. The setting is now stored separately from the plan so it survives page changes and does not affect the deterministic projection.' },
      { kind: 'fix', text: 'Optimizer explain column: conversion notes for optimizer-driven years corrected. The WHY column was showing bracket room and headroom figures that did not match the actual conversion amounts for several common optimizer scenarios.' },
      { kind: 'cosmetic', text: 'Cash Flow Sankey chart redesigned: tax ribbons now flow from each source in proportion to that source\'s share of federal taxable income, so Roth withdrawals never feed a tax bar and pre-tax money carries the tax it actually generates. Roth conversions get a dedicated ribbon to the Roth bucket. ACA premiums appear as their own use node. Labels are hidden for bars too small to read.' },
    ],
  },
  {
    version: '2.0.2',

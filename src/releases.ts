...
export const RELEASES: Release[] = [
  {
    ...
    changes: [
      ...
      { kind: 'fix', text: 'Roth Conversion Benefit now compares against a properly adapted no-conversion baseline — one that re-optimizes the withdrawal strategy — rather than an arbitrary counterfactual. The Roth Conversion Impact chart also switches to after-tax balances so the gap at the end of the plan matches the benefit figure exactly. Changing conversion settings now clears the prior optimizer result (with a notification) so the comparison stays valid.' },
      ...
      { kind: 'cosmetic', text: 'A note under the Expenses section (and in the help guide) clarifies that federal and state taxes, Medicare IRMAA surcharges, and ACA premiums should not be entered as expense rows — FireOpt calculates them automatically.' },
      { kind: 'cosmetic', text: 'The Portfolio section header now shows "balances as of Jan 1, [year]" so it is clear that opening balances should reflect the January 1 starting position, matching how the engine runs a full-year simulation.' },
    ],
  },
  {
    version: '1.8.0',

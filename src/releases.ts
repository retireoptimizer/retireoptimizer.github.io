...
export const RELEASES: Release[] = [
  {
    ...
    changes: [
      ...
      { kind: 'feature', text: 'Optimizer preset strategies (bracket-fill, Roth-first, and others) are now fully considered alongside the main search. Previously they were discarded even when they outperformed the optimizer result.' },
      ...
      { kind: 'feature', text: 'Optimizer explain column: the WHY column now shows correct bracket room and headroom figures for optimizer-driven conversion years.' },
      { kind: 'feature', text: 'Monte Carlo equity mix setting now stays put when you navigate away and return. It was resetting to 60% on each visit.' },
      { kind: 'feature', text: 'Cash Flow chart redesigned: tax flows now come from the accounts that actually generated them, Roth conversions get their own ribbon, and ACA premiums appear as a separate use node.' },
      { kind: 'fix', text: 'Optimizer freshness: if you change your plan after running the optimizer, a gate now blocks the results pages and asks you to re-run or discard the outdated strategy. Previously, a stale optimizer result was silently applied to changed inputs.' },
      { kind: 'fix', text: 'New York state tax: three corrections applied. The $20,000 per-person retirement income exclusion is now honored. The NY standard deduction is applied before bracket math. NY progressive brackets (4% to 9.65%) replace the prior flat 6.5% estimate. For a typical retired NY couple this can reduce projected state tax by $3,000 to $5,000 a year.' },
    ],
  },
  {
    version: '2.0.2',

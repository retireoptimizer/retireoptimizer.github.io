...
export const RELEASES: Release[] = [
  {
    version: '2.0.0',
    date: '2026-08-25',
    summary: 'Age model consolidation: Plan Through Age is now the mortality age; stream EndRule with four Until modes; survivor-benefit fixes; plan warnings strip.',
    changes: [
      { kind: 'feature', text: 'Plan Through Age now serves as both the mortality age and the plan horizon for each person. The old Passing Age field has been removed. Migration v24 preserves both old intents: per person Plan Through Age = max(retirementAge, old passingAge); if the household horizon shrinks, the longer-lived person is extended to restore it. No more "Past Plan" simulation years where spending charged with no income.' },
      { kind: 'feature', text: 'Stream Until control: four EndRule modes replace the bare "Stop age" number — At age (explicit age), End of life (owner\'s Plan Through Age), Last survivor (max Plan Through Age across persons), and For N years (period-certain). Existing streams migrated to "At age" with their old stop age preserved.' },
      { kind: 'feature', text: 'Survivor % column on income and expense streams: sets what fraction of the stream continues after the owner\'s death. 0% for individual life streams, 50% for joint-and-50% annuity, 100% for expenses that continue at full cost.' },
      { kind: 'feature', text: 'Stream row expander (▸ chevron): reveals the resolved window in calendar years. Income streams additionally expose State Taxable % in the expanded panel, freeing up horizontal space in the main row.' },
      { kind: 'feature', text: 'Plan warnings strip: a banner above the panels detects B-tagged streams without a configured Person B, and streams whose stop age precedes their start age — both previously silent modeling errors.' },
      { kind: 'fix', text: 'SS survivor benefit was zero when the deceased person had an explicit SS stream: the stream\'s window check failed once we passed the deceased\'s Plan Through Age, returning 0 instead of the clamped-at-death benefit. The survivor-max comparison now clamps the A-frame age to the deceased\'s Plan Through Age before the window check.' },
      { kind: 'fix', text: 'Phantom SS income from PIA fallback: if a user configured an SS stream starting at 70 but ssClaimAge was 67, the engine emitted 3 phantom PIA-based years of SS (ages 67–69) because stream > 0 was used as the gate. Now hasSSStreamFor() gates the fallback — if any SS stream exists for a person, zero before the window opens is intentional.' },
      { kind: 'fix', text: 'Whose selector in income, expense, and lump-sum rows no longer shows "Person B" as an option when no Person B is configured.' },
      { kind: 'cosmetic', text: 'Page max width increased from 960 to 1024 px to accommodate the wider stream rows.' },
      { kind: 'cosmetic', text: 'How Ages Work section added to the How-To Guide, explaining Plan Through Age semantics, the four Until modes, and Survivor %.' },
    ],
  },
  {
    version: '1.9.0',
    date: '2026-08-22',
    summary: 'Tax-adjusted ending balance and tax-exempt income: optimizer maximizes after-tax portfolio value; Muni Bond and VA/Disability stream types; brokerage exempt yield.',
    changes: [
      ...
      { kind: 'feature', text: 'New income stream types: Muni Bond and VA / Disability. Muni Bond income is federally tax-exempt (IRC §103) but correctly flows through Social Security provisional income, ACA MAGI (IRC §36B), and IRMAA MAGI (42 U.S.C. §1395r(i)(4)). VA / Disability income is fully exempt from federal and state tax (38 U.S.C. §5301) and invisible to every tax surface.' },
      { kind: 'feature', text: 'Tax-Exempt Yield field in Portfolio: enter the fraction of your brokerage return that comes from muni interest. Like dividend yield, it is reinvested into cost basis (IRC §1012) to prevent double-taxation on future withdrawals, while still flowing through MAGI surfaces that require it.' },
      { kind: 'feature', text: 'Double-count guardrail: a warning appears in both Portfolio and Income & Expenses when you have both a tax-exempt portfolio yield and a Muni Bond income stream active simultaneously.' },
      { kind: 'fix', text: 'Non-taxable income was silently dropped from spendable cash. Any income stream with taxablePct < 1 was contributing only its taxable portion to withdrawals and spending. The engine now uses the full gross amount for cash flow while passing only the taxable portion through tax calculations.' },
      { kind: 'fix', text: 'IRMAA MAGI and ACA MAGI now correctly include tax-exempt interest per statute, while NIIT MAGI and OBBBA senior-bonus phase-out continue to exclude it.' },
      { kind: 'fix', text: 'Two off-objective reads in the optimizer (bracketfill competitor check, clamped candidate scoring in min-retirement-age) were routing through endTotalReal instead of the spec scorer. Both now route through the correct objective function.' },
      { kind: 'cosmetic', text: 'Years Funded tile removed from Plan Summary (redundant with the status badge). The badge now includes the years ratio (e.g., "✓ Fully Funded · 30 yrs").' },
      { kind: 'cosmetic', text: 'Portfolio yield fields redesigned: a full-width composition strip below the three return fields labels each yield as a sub-component of Taxable Return.' },
    ],
  },
  {
    version: '1.7.1',

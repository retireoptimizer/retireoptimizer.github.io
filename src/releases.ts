...
export const RELEASES: Release[] = [
  {
    version: '2.1.0',
    date: '2026-09-17',
    summary: 'Monte Carlo overhaul with risk-posture controls and robustness comparison, Roth conversion accuracy fix eliminating phantom round-trips, optimizer policy freshness gating, New York state tax accuracy, and five targeted fixes.',
    changes: [
      { kind: 'feature', text: 'Monte Carlo page redesigned as a two-step flow: Simulate then Optimize. Step 2 is locked until you run a simulation, preventing the optimizer from running against a stale scenario. After optimizing, a before/after success-rate comparison shows exactly how many percentage points the optimizer added — and the result survives navigation so you can review it on the Dashboard.' },
      { kind: 'feature', text: 'Risk posture controls on the Monte Carlo page: choose Floor (prioritize never running out), Balanced, or Growth (maximize expected outcome). The optimizer weights tail risk differently for each posture using a CVaR utility function, so the recommended strategy reflects your actual tolerance for downside scenarios rather than always chasing the highest average.' },
